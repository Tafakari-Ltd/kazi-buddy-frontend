...
type WorkerApplicationsCache = Record<
  string,
  {
...
const AllWorkersAdministration: React.FC = () => {
  ...
  const [workerApplications, setWorkerApplications] =
    useState<WorkerApplicationsCache>({});
  // Default to viewing workers and their applications first
  const [viewMode, setViewMode] = useState<"by_worker" | "by_status">(
    "by_worker",
  );
  ...
  const enrichApplicationsWithJobDetails = async (
    applications: JobApplicationWithDetails[],
  ) => {
    ...
    for (let i = 0; i < enrichedApps.length; i++) {
      ...
      if (!app.job_details && app.job && typeof app.job === "string") {
        try {
          ...
          console.warn(
            `Failed to fetch job details for job ${app.job}:`,
            error,
          );
        }
      }
      ...
      if (workerId) {
        setWorkerApplications((prev) => ({
          ...
          [workerId]: {
            ...prev[workerId],
            applications:
              prev[workerId]?.applications.map((app) =>
  ...
  const getVerificationColor = (status: string) => {
    const option = VERIFICATION_STATUS_OPTIONS.find(
  ...
  const fetchAllApplications = async () => {
    ...
    try {
      ...
      const allApps =
        resp.applications as unknown as JobApplicationWithDetails[];
      const enrichedData = await enrichApplicationsWithJobDetails(
        allApps || [],
      );
    ...
    if (!expanded[id] && !workerApplications[id]?.loaded) {
      ...
      try {
        ...
        if (allApps && allApps.length > 0) {
          console.log(
            "API Response - First application structure:",
            allApps[0],
          );
        ...
        const filteredData = (allApps || []).filter((app) => {
          ...
          const workerId =
            typeof app.worker === "string" ? app.worker : app.worker?.id;
          return workerId === id || app.worker_details?.id === id;
        });

        const enrichedData =
          await enrichApplicationsWithJobDetails(filteredData);
  ...
  const expandAll = async () => {
    ...
    try {
      ...
      const enrichedAllApps = await enrichApplicationsWithJobDetails(
        allApps || [],
      );
            ...
            {viewMode === "by_status" && (
              <div className="flex gap-2 mt-3 flex-wrap">
                ...
                >
                  Reviewed (
                  {
                    allApplications.filter((a) => a.status === "reviewed")
                      .length
                  }
                  )
                </button>
                            ...
                            <h3 className="text-lg font-semibold text-gray-900">
                              {typeof application.worker === "string"
                                ? application.worker_details?.full_name ||
                                  `Worker ID: ${application.worker}`
                                : application.worker?.user?.full_name ||
                                  application.worker_details?.full_name ||
                                  "Unknown Worker"}
                            </h3>
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full flex items-center gap-1 ${getApplicationStatusColor(application.status)}`}
                            >
                        ...
                        <div className="flex items-center gap-2">
                          {application.status === "pending" && (
                            <>
                              <button
                                onClick={() =>
                                  handleMoveToReviewed(application.id)
                                }
                                disabled={processingApplicationIds.has(
                                  application.id,
                                )}
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-semibold text-gray-900">
                                  {typeof profile.user === "string"
                                    ? `Worker ID: ${profile.user}`
                                    : profile.user?.full_name ||
                                      `Worker #${profile.id}`}
                                </span>
                                <span
                                  className={`px-2 py-0.5 rounded-full text-xs font-medium ${getVerificationColor(profile.verification_status)}`}
                                >
                                  {VERIFICATION_STATUS_OPTIONS.find(
                                    (opt) =>
                                      opt.value === profile.verification_status,
                              ...
                              <div className="mt-1 text-sm text-gray-600 flex flex-wrap items-center gap-4">
                                ...
                                <span
                                  className={`inline-flex items-center gap-1 ${profile.is_available ? "text-green-600" : "text-red-600"}`}
                                >
                        ...
                        {isOpen && (
                          <div className="px-10 pb-5">
                            ...
                            {cache?.loading ? (
                              ...
                              <div className="text-gray-600">
                                No job applications from this worker.
                              </div>
                            ) : (
                                ...
                                {cache.applications.map((application) => (
                                  ...
                                  >
                                    <div className="flex items-center justify-between mb-1">
                                      <h4 className="font-semibold text-gray-900 truncate pr-2">
                                        {(typeof application.job !== "string"
                                          ...
                                          application.job_details?.title ||
                                          (application.job
                                            ? `Job ID: ${typeof application.job === "string" ? application.job : application.job.id}`
                                            : "Unknown Job")}
                                      </h4>
                                      <span
                                        className={`text-xs px-2 py-0.5 rounded-full flex items-center gap-1 ${getApplicationStatusColor(application.status)}`}
                                    ...
                                    <div className="flex items-center justify-between">
                                      ...
                                      <button
                                        onClick={() =>
                                          openViewApplication(application)
                                        }
                                        className="text-blue-600 hover:text-blue-800 text-sm font-medium"
                                      >
                                        View Details
              ...
              >
                <div className="flex justify-between items-start mb-6 pb-4 border-b border-gray-200">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1 ${getApplicationStatusColor(applicationToView.status)}`}
                      >
                        {getApplicationStatusIcon(applicationToView.status)}
                        {applicationToView.status.charAt(0).toUpperCase() +
                          applicationToView.status.slice(1)}
                      </span>
                    </div>
                  </div>
                  <button
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <h5 className="font-medium text-blue-900">
                            {(typeof applicationToView.job !== "string"
                              ? applicationToView.job?.title
                              : undefined) ||
                              applicationToView.job_details?.title ||
                              (applicationToView.job
                                ? `Job ID: ${typeof applicationToView.job === "string" ? applicationToView.job : applicationToView.job.id}`
                                : "Unknown Job")}
                          </h5>
                        </div>
                        <p className="text-blue-800 text-sm mb-2">
                          {(typeof applicationToView.job !== "string"
                            ? applicationToView.job?.employer?.company_name
                            : undefined) ||
                            applicationToView.employer_details?.company_name ||
                            "Unknown Company"}
                        </p>
                        <div className="flex items-center gap-4 text-sm text-blue-700">
                          <span className="inline-flex items-center gap-1">
                            <MapPin size={14} />
                            {(typeof applicationToView.job !== "string"
                              ? applicationToView.job?.location
                              : undefined) ||
                              applicationToView.job_details?.location}
                          </span>
                          {((typeof applicationToView.job !== "string"
                            ? applicationToView.job?.budget_min &&
                              applicationToView.job?.budget_max
                            : false) ||
                            (applicationToView.job_details?.budget_min &&
                              applicationToView.job_details?.budget_max)) && (
                            <span className="inline-flex items-center gap-1">
                              <DollarSign size={14} />
                              {formatCurrency(
                                (typeof applicationToView.job !== "string"
                  ...
                  <div className="space-y-6">
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="text-gray-900">
                            {formatDate(applicationToView.applied_at)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Available From:</span>
                          <span className="text-gray-900">
                            {formatDate(applicationToView.availability_start)}
                          </span>
                        </div>
                        {applicationToView.responded_at && (
                          <div className="flex justify-between">
                            ...
                            <span className="text-gray-900">
                              {formatDate(applicationToView.responded_at)}
                            </span>
                          </div>
                        )}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-sm">
                        {((typeof applicationToView.worker !== "string"
                          ? applicationToView.worker?.user?.full_name
                          : undefined) ||
                        ...
                        {((typeof applicationToView.worker !== "string"
                          ? applicationToView.worker?.user?.email
                          : undefined) ||
                  ...
                  <div className="flex gap-2">
                    {applicationToView.status === "pending" && (
                      <>
                        <button
                          onClick={() => {
                            const workerIdString =
                              typeof applicationToView.worker === "string"
                                ? applicationToView.worker
                                : applicationToView.worker_details?.id;
                            const workerId = profiles.find(
                              (p) =>
                                (typeof p.user === "string"
                                  ? p.user
                                  : p.user?.id) === workerIdString ||
                                p.id === workerIdString,
                            )?.id;
                            if (workerId)
                              handleMoveToReviewed(
                                applicationToView.id,
                                workerId,
                              );
                          }}
                          disabled={processingApplicationIds.has(
                    ...
                    {(applicationToView.status === "accepted" ||
                      applicationToView.status === "rejected") && (
                      <div className="text-sm text-gray-500 flex items-center">
                        ...
                        {applicationToView.responded_at && (
                          <span className="ml-2">
                            on {formatDate(applicationToView.responded_at)}
                          </span>
                        )}
                      </div>
                    )}
