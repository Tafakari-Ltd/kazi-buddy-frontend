...
type EmployerJobsCache = Record<
  string,
  { jobs: Job[]; loading: boolean; error: string | null; loaded: boolean }
>;
...
const AllEmployersAdministration: React.FC = () => {
  ...
  // Default to viewing employers and their jobs first
  const [viewMode, setViewMode] = useState<"by_employer" | "by_status">(
    "by_employer",
  );
  ...
  const fetchPendingJobs = async () => {
    try {
      ...
      const ids = new Set<string>(
        (raw as any[]).map((j) => j.id).filter(Boolean),
      );
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full ${getUrgencyColor(job.urgency_level)}`}
                            >
                              {URGENCY_LEVEL_OPTIONS.find(
                                (opt) => opt.value === job.urgency_level,
                              )?.label || job.urgency_level}
                            </span>
                          </div>
                  ...
                  <div className="space-y-6">
                    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 p-4 rounded-lg">
                      ...
                      <p className="text-2xl font-bold text-blue-900">
                        {formatCurrency((jobToView as any).budget_min)} -{" "}
                        {formatCurrency((jobToView as any).budget_max)}
                      </p>
