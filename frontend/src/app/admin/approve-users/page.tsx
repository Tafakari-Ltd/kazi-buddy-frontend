...
const ApproveUsersPage: React.FC = () => {
 ...
 if (err.status === 404) {
 errorMessage = 'The pending users endpoint is not available. Please contact your backend team to implement GET /api/adminpanel/users/pending/';
 toast.error('Endpoint not found - contact backend team', { duration: 5000 });
 ...
 const rowId = getPendingUserId(user) ?? `${user.email}-${idx}`;
 return (
 <tr
 key={rowId}
 ...
 disabled={loading || approvingId === getPendingUserId(user)}
